...
int main(int argc, char* argv[])
{
	...
	CfgPubImg cfgPub = cfg->getPubImg();

	const std::string logFilename = std::string("log-pub-") + cfgPub.topicName + ".txt";
	const bool showInConsole = true;
	startLogThread(logFilename, showInConsole);

	bool use_environment_qos = false;

	std::shared_ptr<CfgPubImg> cfgPubImg( new CfgPubImg(cfgPub) );
	ImgPub mypub(cfgPubImg);
