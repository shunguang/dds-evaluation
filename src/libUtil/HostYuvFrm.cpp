...
{
#if DEBUG_
	APP_ASSERT( bufSz == (w*h*3)/2, "HostYuvFrm::HostYuvFrm(): size does not match!");
#endif
}
...
HostYuvFrm::HostYuvFrm(const HostYuvFrm &x)
	...
	, buf_(NULL)
{
	//APP_LOG("HostYuvFrm::HostYuvFrm(const HostYuvFrm &x) called, x.fn=%d",x.fn_);
	creatBuf();
...
void HostYuvFrm::creatBuf()
{
	if( sz_ > 0 ){
		...
		//APP_LOG("HostYuvFrm::creatBuf() called, fn_=%lld, sz_=%ld", fn_, sz_);
		if (!buf_) {
			APP_LOG("HostYuvFrm::creatBuf(): cannot allocate memory!");
			assert(0);
		}
	}
	else {
...
{
	APP_ASSERT(w_ == bgr.cols && h_ == bgr.rows, "HostYuvFrm::hdCopyFromBgr(): size dose not match!");
